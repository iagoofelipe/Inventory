...
namespace inventory
{
		...
		: wxPanel(parent, id)
	{
		...
		btnSave->Bind(wxEVT_BUTTON, [&](wxCommandEvent&) { dispatchEvent(EVT_CONFIGVIEW_SAVE, this); });
	}
	...
	void ConfigView::SetValue(const DatabaseConnParams& cfg)
	{
	...
	void ConfigView::GetValue(DatabaseConnParams& cfg)
	{
		cfg.host = txtHost->GetValue();
		cfg.port = scPort->GetValue();
		cfg.schema = txtSchema->GetValue();
		cfg.user = txtUser->GetValue();
		cfg.password = txtPassword->GetValue();
	}
}